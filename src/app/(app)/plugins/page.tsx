    ...
    <div>
      ...
      {plugins.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {plugins.map((p) => (
            <Card key={p.manifest.id} className="p-5">
              <div className="mb-2 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h3 className="font-semibold">{p.manifest.name}</h3>
                    <p className="font-mono text-[11px] text-muted">{p.manifest.id}</p>
                  </div>
                </div>
                <Badge color={p.enabled ? "#22c55e" : "#64748b"}>
                  v{p.manifest.version} - {p.enabled ? "ativo" : "inativo"}
                </Badge>
              </div>
              {p.manifest.description && <p className="text-sm text-muted">{p.manifest.description}</p>}
              <div className="mt-4 flex flex-wrap gap-2 text-xs">
                <span className="inline-flex items-center gap-1 rounded-md bg-surface2 px-2 py-1"><Wrench size={12} /> {p.manifest.aiTools?.length ?? 0} ferramentas IA</span>
                <span className="inline-flex items-center gap-1 rounded-md bg-surface2 px-2 py-1"><LayoutGrid size={12} /> {p.manifest.ui?.length ?? 0} contribuicoes UI</span>
                <span className="inline-flex items-center gap-1 rounded-md bg-surface2 px-2 py-1"><Radio size={12} /> {p.manifest.subscriptions?.length ?? 0} eventos</span>
              </div>
              {p.manifest.aiTools && p.manifest.aiTools.length > 0 && (
                <div className="mt-3 border-t border-border pt-3">
                  <p className="mb-1 text-[11px] font-medium text-muted">Ferramentas de IA:</p>
                  {p.manifest.aiTools.map((t) => (
