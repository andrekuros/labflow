...
export function KnowledgeFolderTree({ tree, localCount, selected, onSelect }: Props) {
  return (
    <div className="rounded-xl border border-border bg-surface p-3">
      ...
      {tree.map((node) => (
        <TreeNode key={node.path || node.name} node={node} selected={selected} onSelect={onSelect} />
      ))}
      {localCount > 0 && (
        <button
          type="button"
